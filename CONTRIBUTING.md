...
Open an issue at [github.com/tessera-llm/tessera-autogen/issues](https://github.com/tessera-llm/tessera-autogen/issues) with:

- Package version (`pip show tessera-autogen`)
- AutoGen version
...
This package is a thin adapter on top of the AutoGen framework. The Tessera wrapper exposes a one-line config helper that routes AutoGen's LLM calls through the Tessera proxy at `api.tesseraai.io`. Keep new helpers tiny -- one config-factory per upstream provider, no business logic inside the wrapper.
